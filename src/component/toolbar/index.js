...
export default class Toolbar {
	constructor(data, sheet, widthFn, isHide = false) {
		...
		// added code
		const {formatEl} = this
		this.isLocale = {is: false}
		const {isLocale} = this

		this.translationEl.setClick(function () {
			isLocale.is = !isLocale.is;

			if (isLocale.is) {
				Spreadsheet.locale("zhCn", zhCn)
			} else {
				Spreadsheet.locale("en", en)
			}
			formatEl.el.el.childNodes[0].firstChild.firstChild.innerText = tf(`format.${formatEl.value}`)()
		})
	}

...
const menuItemMap =new Map(menuItems.map(it => [it.key, it.title]));
...
class ContextMenu {
	...
	setOffset(offset) {
		this.el.el.childNodes.forEach(it => {
			const attr = it.getAttribute('key');
			it.innerText = menuItemMap.get(attr)();
		})
		const {el} = this;
		el.offset(offset);
		el.show();
